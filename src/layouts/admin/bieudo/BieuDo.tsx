...
import {HighlightScope, PieChart} from "@mui/x-charts";
...
interface BieuDoProps {
    dataTotalPriceOrderByMonth?: number[];
    dataNumberOfOrderOrderByMonth?: number[];
    danhSachDonHang?: DonHangModel[];
}

export const BieuDo: React.FC<BieuDoProps> = (props) => {
        ...
        () => (option: string, year?: string, latestDays?: string[]) => {
            switch (option) {
                case "yearly":
                    ...
                    props.danhSachDonHang?.forEach((order) => {
                ...
                case "monthly":
                    ...
                    props.danhSachDonHang?.forEach((order) => {
                        ...
                        if (
                            ...
                            newDataTotalPrice_Monthly[month] += order.tongTien;
                        }
                    });
    ...
    useEffect(() => {
        const orderTotal = props.danhSachDonHang?.length ? props.danhSachDonHang.length : 0;
        ...
        props.danhSachDonHang?.forEach((order) => {
            ...
            } else if (order.trangThai === "Bị huỷ") {
                orderFailureTotal++;
